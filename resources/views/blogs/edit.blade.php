    ...
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('sections.index') }}">Blogs</a></li>
                ...
                <form class="row g-3" action="{{ route('blogs.update')}}" method="POST">
                    ...
                    <div class="col-md-6">
                        <div class="form-floating">
                            <input name="title" class="form-control" placeholder="Título" value="{{ $blog->title }}">
                            <label>Título</label>
                        </div>
                    </div>
                            ...
                            <select class="form-control" name="section_id">
                                @foreach ($sections as $section)
                                    <option value="{{ $section->id }}"
                                            {{ $section->id == $blog->section_id ? 'selected' : '' }} >
                                            {{ $section->name }}
                                    </option>
                                @endforeach
                            </select>
